...
abstract class WC_maxiPago_API {
			...
			if ( $order->get_id() === $order_id ) {
				...
				switch ( $order_status ) {
						...
						if ( $processing_type == WC_maxiPago_CC_Gateway::PROCESSING_TYPE_SALE ) {
							...
							if ( $authResponse == 'CAPTURED' ) {
								...
								$order->update_status( 'on-hold',
									__( 'maxiPago!: This order requires manual capture.', 'modulo-woocommerce' ) );
							}
						} else {
							if ( $order->get_payment_method() == WC_maxiPago_Boleto_Gateway::ID ) {
								$order->update_status( 'on-hold',
									__( 'maxiPago!: After we receive the boleto payment confirmation, your order will be processed.',
										'modulo-woocommerce' ) );
							} elseif ( $order->get_payment_method() == WC_maxiPago_Online_Debit_Gateway::ID ) {
								$order->update_status( 'on-hold',
									__( 'maxiPago!: After we receive the Online Debit payment confirmation, your order will be processed.',
										'modulo-woocommerce' ) );
							} else {
								$order->update_status( 'on-hold',
									__( 'maxiPago!: This order requires manual capture.', 'modulo-woocommerce' ) );
					...
					case 5 : // Revisão de Fraude
						$order->update_status( 'on-hold',
							__( 'maxiPago!: This order is under fraud review, please wait for return.',
								'modulo-woocommerce' ) );
						$valid = true;
						break;
					case 1 : // Negada
					...
					case 4097 : // Erro interno na maxiPago!
						$order->update_status( 'failed',
							__( 'maxiPago!: An error has occurred while processing your payment.',
								'modulo-woocommerce' ) );
						$valid = true;
						break;
				}
	...
	public function get_banks( $type = 'boleto' ) {
		$banks = array(
			'boleto'       => array(
				''   => 'Selecione o banco',
				'13' => 'Banco do Brasil',
