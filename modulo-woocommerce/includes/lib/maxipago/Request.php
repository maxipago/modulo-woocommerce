...
class maxiPago_Request extends maxiPago_XmlBuilder {
	...
	private function debugger( $string ) {
		...
		echo( "<br>" . str_repeat( "-", 20 ) . "<br>[" . $_d . "] " . $string . "<br>" . str_repeat( "-",
				20 ) . "<br>" );
	}
			...
			foreach ( $xmlResponse->children() as $key => $value ) {
				if ( $key == "header" ) {
					...
					if ( ! empty( $resultSetInfo ) ) {
						foreach ( $resultSetInfo as $resultSetKey => $resultSetValue ) {
							$array[ $resultSetKey ] = (string) $resultSetValue;
						}
						$records = $xmlResponse->result->records[0];
						foreach ( $records as $recordKey => $recordVal ) {
							foreach ( $recordVal as $k => $v ) {
