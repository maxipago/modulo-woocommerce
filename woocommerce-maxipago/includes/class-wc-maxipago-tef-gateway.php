...
class WC_maxiPago_TEF_Gateway extends WC_Payment_Gateway_CC {
    ...
    public function __construct() {
        ...
        $this->method_description = __('Accept Payments by TEF using the maxiPago!', 'woocommerce-maxipago');
        ...
        $this->api = new WC_maxiPago_TEF_API($this);
    ...
    public function init_form_fields() {
        ...
        $this->form_fields = array(
            'enabled' => array(
                ...
                'type' => 'checkbox',
                'label' => __('Enable maxiPago! TEF', 'woocommerce-maxipago'),
                'default' => 'no',
            ),
            'title' => array(
                ...
                'desc_tip' => true,
                'default' => __('TEF', 'woocommerce-maxipago')
            ),
            'description' => array(
                ...
                'description' => __('Displayed at checkout.', 'woocommerce-maxipago'),
                'desc_tip' => true,
                'default' => __('Pay your order with a TEF.', 'woocommerce-maxipago')
            ),
            ...
            'banks' => array(
                'title' => __('Banks', 'woocommerce-maxipago'),
                'type' => 'multiselect',
                'description' => __('Choose your banks.', 'woocommerce-maxipago'),
                'desc_tip' => true,
                'class' => 'wc-enhanced-multiselect',
                'default' => '',
                'options' => $this->api->get_banks('tef')
            ),
        );
    }
    ...
    public function set_email_instructions(WC_Order $order, $sent_to_admin, $plain_text = false) {
        ...
        if (isset($result_data['boletoUrl'])) {
            if ($plain_text) {
                wc_get_template(
                    'tef/emails/plain-instructions.php',
                    array(
                        'url' => $result_data['onlineDebitUrl'],
                    ),
                    'woocommerce/maxipago/',
                ...
                wc_get_template(
                    'tef/emails/html-instructions.php',
                    array(
                        'url' => $result_data['onlineDebitUrl'],
                    ),
                    'woocommerce/maxipago/',
